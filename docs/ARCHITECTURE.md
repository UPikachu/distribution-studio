...
平台导航统一由 `electron/platform-navigation.ts` 处理。九个平台沿用各自的域名白名单；白名单内的标准 HTTP 页面地址先升级为 HTTPS，保留路径、查询参数和片段，不发送到明文页面。不接受带账号密码或非标准 HTTP 端口的地址，也不因登录失败扩大域名范围。

主窗口导航、重定向和弹窗入口共用这项策略。请求层仅处理顶层 GET 页面，不改写 POST、接口或资源请求。知乎登录页的 `next` 和百家号 stoken 页的 `u` 属于已确认的特殊回调，仅将其中可信的 HTTP 返回地址升级；未知参数和已经是 HTTPS 的回调保持原样，避免破坏签名。

拦截提示明确区分客户端限制与平台风控，只显示目标站点，不显示查询参数、片段或登录凭证。遇到新问题应先确认具体跳转地址，再补充最小范围规则与回归测试；不能直接关闭导航限制或放行所有域名。
